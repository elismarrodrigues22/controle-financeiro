        ...
        db.collection("usuarios")
            ...
            .then(doc => {
                ...
                if (!doc.exists) {
                    ...
                    planoUsuario = doc.data().plano || "free";
                }

                mostrarSistema();
                carregarTransacoes();
                atualizarBadgePlano();
                controlarBotaoUpgrade();
                atualizarAvisoPlano();
...
function carregarTransacoes(mesSelecionado = null, categoriaSelecionada = "") {
    ...
    db.collection("usuarios")
        ...
        .onSnapshot(snapshot => {
            ...
            animarValor(document.getElementById("saldo"), saldo);
            animarValor(document.getElementById("entradas"), entradas);
            animarValor(document.getElementById("saidas"), saidas);

            if (atingiuLimiteFree()) {
...
function aplicarFiltros() {
    ...
    mostrarMensagem(
        "Plano Pro libera transações ilimitadas, gráficos avançados e suporte prioritário 🚀"
    );
});
...
function animarValor(elemento, valorFinal) {
    ...
    const intervalo = setInterval(() => {
        ...
        elemento.innerText = `R$ ${valorAtual.toFixed(2)}`;
    }, 20);
}
...
function atingiuLimiteFree() {
    return transacoesCache.length >= LIMITE_FREE;
}
...
function atualizarBadgePlano() {
    ...
    badge.innerText = planoUsuario === "free" ? "Plano Free" : "Plano Pro";
}

function controlarBotaoUpgrade() {
...
function atualizarAvisoPlano() {
    const aviso = document.querySelector(".upgrade");
    if (!aviso) return;
